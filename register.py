...
def register():
    if entryfirstname.get() == '' or entrylastname.get() == '' or entryemail.get() == '' or entrycontact.get() == '' or \
        ...
        try:
            ...
            cur = con.cursor()
            cur.execute('select * from login where email=%s', entryemail.get())
            row = cur.fetchone()
            if row is not None:
                ...
                cur.execute(
                    'insert into login (f_name,l_name,email,contact,question,answer,password) values(%s,%s,%s,%s,%s,'
                    '%s,%s)',
...
button = PhotoImage(file='resource/Images/button.png')
...
loginimage = PhotoImage(file='resource/Images/login.png')
